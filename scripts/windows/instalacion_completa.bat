@echo off
title Instalador Completo GymManager
color 0A
echo ====================================================================
echo                 INSTALADOR COMPLETO DE GYMMANAGER
echo ====================================================================
echo.
echo Este script instalara todo lo necesario para ejecutar GymManager.
echo No se requiere ninguna instalacion previa.
echo.
...
if %errorLevel% neq 0 (
    ...
    powershell -Command "& {Invoke-WebRequest -Uri 'https://www.python.org/ftp/python/3.8.10/python-3.8.10-amd64.exe' -OutFile 'C:\GymManager\Temp\python-installer.exe'}"
...
:: Descargar el repositorio de GymManager
...
if /i "%AUTO_START%"=="S" (
    powershell -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\GymManager.lnk'); $Shortcut.TargetPath = 'C:\GymManager\scripts\windows\run.bat'; $Shortcut.WorkingDirectory = 'C:\GymManager'; $Shortcut.Save()"
